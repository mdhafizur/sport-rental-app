  ...
  <div class="q-pa-md row justify-center">
    <div class="col-10 col-sm-6 col-md-6 col-lg-4">
      <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
        <q-input filled v-model="inventory.title" label="Inventory Name *" hint="Name of the inventory" lazy-rules
        ...
        <q-select filled v-model="inventory.item" :emit-value="false" :options="options.data" label="Label (stacked)"
          stack-label :dense="dense" :options-dense="denseOpts" />

        <q-input filled type="number" v-model="inventory.quantity" label="inventory quantity *" step="any" lazy-rules
          :rules="[
...
onMounted(async () => {
  ...
  options.data = itemsStore.getItems.map((item) => ({ label: item.title, value: item.id }));
